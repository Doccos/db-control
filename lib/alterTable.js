...
class AlterTable {
                ...
                for (let key of keys.keys) {
                    ...
                    for (let aKeys of this._availablelKeys[this._newTable.tableName].primaryKey) {
                        for (let aKey of aKeys.keys) {
                            if (aKey.name === key.name && aKey.length == key.length)
                                found = true;
                        }
                    }
        ...
        for(let keyType of ['uniqueKey', 'fulltextKey', 'key'])
        {
            if(this._availablelKeys[this._newTable.tableName] && this._availablelKeys[this._newTable.tableName][keyType] && this._availablelKeys[this._newTable.tableName][keyType].length > 0 && this._newTable.keys[keyType].length > 0) {
                ...
                for(let key of this._newTable.keys[keyType]){
                    ...
                    if(!found){
                        let query = 'ALTER TABLE `' + this._dbName + '`.`' + this._newTable.tableName + '` ADD '+trans[keyType]+' `'+key.keyName+'` (' + key.getKeyString() + ')';
                        this._queryPool.alterKeyColumn.push({query: query});
                    }
                ...
                for(let key of this._newTable.keys[keyType]){
                    let allFound = true;
                    // console.log(this._availablelKeys[this._newTable.tableName][keyType]);
                    for(let aKey of this._availablelKeys[this._newTable.tableName][keyType]){
                        if(aKey.keyName === key.keyName){
                           ...
                           for(let col of key.keys){
                               ...
                               for(let aCol of aKey.keys) {
                                   if(col.name === aCol.name && col.length === aCol.length)
                                       found = true;
                               }
                               if(!found)
                                   allFound = false;
                    ...
                    if(!allFound){
                        let query = 'ALTER TABLE `' + this._dbName + '`.`' + this._newTable.tableName + '` DROP KEY `'+key.keyName+'`, ADD '+trans[keyType]+' `'+key.keyName+'` (' + key.getKeyString() + ')';
                        this._queryPool.alterKeyColumn.push({query: query});
                    }
                ...
                for(let key of this._newTable.keys[keyType]) {
                    let query = 'ALTER TABLE `' + this._dbName + '`.`' + this._newTable.tableName + '` ADD '+trans[keyType]+' `'+key.keyName+'` (`' + key.keys.getKeyString() + '`)';
                    this._queryPool.alterKeyColumn.push({query: query});
                }
            } else if(this._availablelKeys[this._newTable.tableName] && this._availablelKeys[this._newTable.tableName][keyType] && this._availablelKeys[this._newTable.tableName][keyType].length >0 && this._newTable.keys[keyType].length === 0) {
    ...
    _compareColumns(column, dbColumn) {
        ...
        if (column.zerofill != dbColumn.zerofill)
            isEqual = false;
        if (column.defaultVal != dbColumn.defaultVal && column.defaultVal !== null && !column.isAutoIncrement )
            isEqual = false;
        if(column.currentTS !== null && dbColumn.currentTS !== column.currentTS)
            isEqual = false;
        if(column.onUpdCurTS !== null && dbColumn.onUpdCurTS !== column.onUpdCurTS)
            isEqual = false;
        if(column.isNullAllowed != dbColumn.isNullAllowed)
            isEqual = false;
        // console.log(column.onUpdCurTS , dbColumn.onUpdCurTS , column.onUpdCurTS);
        // console.log(column.currentTS);
        // console.log(this._newTable.tableName, column.name , column.defaultVal , dbColumn.defaultVal, column.defaultVal , column.isAutoIncrement );
        ...
        if (!isEqual) {
           // console.log(column.isUnsigned ,dbColumn.isUnsigned);
            ...
            query += builder.column(column, column.isAutoIncrement)
        ...
        if (column.defaultVal === null && dbColumn.defaultVal !== null) {
            console.log(typeof column.defaultVal,column.defaultVal,typeof dbColumn.defaultVal,dbColumn.defaultVal, "asd", column, dbColumn);
