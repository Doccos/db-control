...
class Control {
    ...
    async _createAllTables() {
        for (let table in this._tables) {
            this._createTable(this._tables[table])
        }
    }
        ...
        return new Promise(resolve => {
            this._mysql.query("SHOW COLUMNS FROM `" + this._dbName + "`.`" + this._availableTables[id].tableName + "`", (err, rows, fields) => {
                ...
                for (let row of rows) {
                    ...
                    let defaultVal = (row.Default === 'CURRENT_TIMESTAMP') ? null : row.Default;
            ...
            this._mysql.query('SHOW CREATE TABLE `'+this._dbName+'`.`'+table.tableName+'`', (err, rows, fields) => {
                ...
                for(let line of splitted) {
                    ...
                    if(relation === null){
                        ...
                        // if(key && key[2] && key[2].startsWith("K_type_slug"))
                            // console.log(key);
                        // console.log(key);
                        // console.log(line);
                        if(key !== null){
                            let keyM;
                            keyM = new modelKey(key[3].replace(/\`/ig,'').split(','), [], "dbData")
                            keyM.keyName = key[2];
                             // console.log(key[3]);
                            if(key[1] === 'FULLTEXT')
